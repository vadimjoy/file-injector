...
- [ ] Ни один компонентный CSS-файл не содержит правил, таргетирующих классы других компонентов
- [ ] Аудит-скрипт (Node.js) прогоняется в CI и падает при нарушении
- [ ] `ui-input` имеет полный набор явных модификаторов для всех контекстов
- [ ] `ui-datepicker` и `ui-autocomplete` обновлены по новому стандарту
- [ ] Обновлён `SPEC.md` с новыми HTML-контрактами
...
2. Запустить аудит coupling в `datepicker.css` и `autocomplete.css`
