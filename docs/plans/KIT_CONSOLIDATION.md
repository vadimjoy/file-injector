# Kit Consolidation & Visual Cleanup

> ⚠️ **Актуализация плана (2026-04-17):** в MASTER_PLAN эта работа учтена как **подфаза 4.2 «Kit Consolidation»**, вклинивающаяся между подфазой 4.1 (Architecture, ✅) и треком предварительного масштабируемости (A/B/F). Имя файла `KIT_CONSOLIDATION.md` оставлено без изменений, чтобы не ломать ссылки. Все приведённые ниже задачи, критерии и спринты действительны; только нумерация фаз в MASTER_PLAN и SCALABILITY_PLAN переехала с «Phase 6» на «подфазу 4.2». Полная дорожная карта: [`MASTER_PLAN`](../localization/ru/MASTER_PLAN.md).

> **Горизонт:** v0.7.x (между подфазой 4.1 и треками A/B/F; **блокер** для подфаз 4.4–4.7)
> **Статус:** ✅ **Завершена** *(2026-04-17)*
> **Связанные документы:** [`MASTER_PLAN`](../localization/ru/MASTER_PLAN.md) · [`SCALABILITY_PLAN`](./SCALABILITY_PLAN.md) · [`DEMO_PLAYGROUND`](./DEMO_PLAYGROUND.md)
...
- [x] 0 inline-`style=""` в `src/demos/*.html` (кроме отмеченных `demo: token override`)
- [x] 0 захардкоженных hex/rgb в `src/css/components/*.css`
- [x] Каждый BEM-блок живёт в одном файле; CSS-файлов в `components/` = 50 (было 52 + 2 merged-out + 3 new)
- [x] `feed.css`, `banner.css`, `callout.css` — deprecated stub files (alias → `timeline` / `alert`); удалятся в v1.0.0
- [x] `icon-tile.css`, `dot.css`, `foundations/icon.css`, `foundations/color.css` созданы
- [x] `warm.json` и `midnight.json` не изменяют `font.family`
- [x] `feature-item --dark` удалён
- [x] Все 50 компонентов проходят Module Contract v2 (CI) — 4 lint-скрипта ✓
- [x] `index.css` сгруппирован по слоям (foundations → atoms → molecules → organisms)
- [x] `AI_CONTEXT.md` обновлён; `SPEC.md` будет обновлён в рамках Фазы 4.3
- [x] Визуальный diff-ревью: темы исправлены, демо-страницы прошли ревью
- [x] Обновлён `MASTER_PLAN.md` — подфаза 4.2 отмечена как ✅
...
2. ✅ Запустить Sprint 6.1–6.6 (Foundations → Lint CI)
3. → Создать ADR-0006 «Theme purity: color-only theme variation»
4. → Создать ADR-0007 «Icon-tile & dot as foundational atoms»
5. → Запустить Фазу 4.3: [`DEMO_PLAYGROUND`](./DEMO_PLAYGROUND.md) (интерактивные демо)
